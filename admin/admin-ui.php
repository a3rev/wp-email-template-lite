...
class Admin_UI
{
	 ...
	 */
	public $pro_plugin_page_url = 'https://a3rev.com/shop/wp-email-template/';
	...
	public function __construct() {
		...
		$this->google_map_api_key_option = WP_EMAIL_TEMPLATE_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = WP_EMAIL_TEMPLATE_KEY . '_toggle_box_open';
		$this->version_transient         = WP_EMAIL_TEMPLATE_KEY . '_licinfo';

		if ( defined( 'WP_EMAIL_TEMPLATE_G_FONTS' ) ) {
			$this->is_load_google_fonts = (boolean) WP_EMAIL_TEMPLATE_G_FONTS;
		}
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'wp-email-template' ),
				'image_url'    => WP_EMAIL_TEMPLATE_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'wp-email-template' ),
				'left_text'    => __( 'WP Email Template Premium', 'wp-email-template' )
									. "\n\n" . __( 'Quick Video showing the main (not all) enhanced features that are built into the WP Email Template Premium version', 'wp-email-template' ),
				'right_title'  => __( 'Developer Support and Premium Features', 'wp-email-template' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'wp-email-template' )
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">'
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'wp-email-template' )
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Premium Version', 'wp-email-template' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'wp-email-template' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'wp-email-template' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( WP_EMAIL_TEMPLATE_VERSION , $version_info[0], '<' ) ) {

						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'wp-email-template' ),
							$version_info[0],
